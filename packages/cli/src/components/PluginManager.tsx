  ...
  return (
    ...
    >
      ...
      <Box position="absolute" marginTop={-1} marginLeft={2} backgroundColor="black" paddingX={1}>
        <Text color="#AA00FF" bold> NEURAL_EXPANSION:PLUGIN_CORE </Text>
      </Box>

      {/* Filter bar */}
      <Box marginBottom={1} paddingX={1} borderStyle="round" borderColor="#484F58">
        <Text color="#FF00AA" bold>❯ </Text>
        <Text color="#AA00FF" bold>SEARCH_MODULES: </Text>
      ...
      {filtered.length === 0 && (
        <Box paddingY={2} alignItems="center" flexDirection="column">
          <Text color="#FF5555" bold> [!] NO_PLUGINS_ACTIVE </Text>
          ...
          return (
            <Box key={p.manifest.name} paddingLeft={2} backgroundColor={focused ? '#161b22' : undefined}>
              ...
              <Box flexDirection="row" justifyContent="space-between" flexGrow={1}>
                <Box>
                  <Text color={focused ? 'white' : '#8C959F'} bold={focused}>{p.manifest.name.toUpperCase()}</Text>
      ...
      {confirmDelete && (
        <Box
          ...
          borderColor="#FF5555"
        >
          <Text color="#FF5555" bold> UNLINK PLUGIN {confirmDelete.toUpperCase()}? [Y/N] </Text>
        </Box>
      )}

      <Box marginTop={1} justifyContent="space-between" paddingX={1}>
        <Text color="#484F58"> ESC:ABORT │ A:ADD │ ↑↓:NAVIGATE </Text>
        <Text color="#AA00FF" bold> E:EDIT │ BACKSPACE:DELETE </Text>
      </Box>
    </Box>
  );
}
