...
export function ProviderWizard({ providerName, onSave, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    import('@personal-cli/core').then(
      async ({ startDeviceFlow, pollForGitHubToken, saveGitHubToken, isCopilotAuthenticated }) => {
        try {
    ...
    >
      ...
      <Box position="absolute" marginTop={-1} marginLeft={2} backgroundColor="black" paddingX={1}>
        <Text color={info.color} bold>
          {' '}
          INITIALIZING_PROVIDER_LINK_{providerName.toUpperCase()}{' '}
        </Text>
      </Box>
      ...
      <Box flexDirection="row" justifyContent="space-between">
        <Box flexDirection="column" flexGrow={1}>
          <Box marginBottom={1}>
            <Text bold color={info.color} inverse>
              {' '}
              {info.label.toUpperCase()}{' '}
            </Text>
            <Text color={info.color}> ▐ </Text>
            <Text italic color="#8C959F">
              {info.description}
            </Text>
          </Box>
          ...
          {info.keyUrl && (
            <Box>
              ...
              <Text color={info.color} underline>
                {info.keyUrl}
              </Text>
            </Box>
          )}
