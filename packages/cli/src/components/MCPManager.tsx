...
export function MCPManager({ 
  ...
  const filtered = useMemo(() => {
    ...
    const disconnected = servers.filter(s => 
      s.status !== 'connected' && 
      s.name.toLowerCase().includes(query)
    );
  ...
  return (
    ...
    >
      ...
      <Box position="absolute" marginTop={-1} marginLeft={2} backgroundColor="black" paddingX={1}>
        <Text color="#00E5FF" bold> NEURAL_LINK:MCP_SERVERS </Text>
      </Box>
      ...
      <Box marginBottom={1} paddingX={1}>
        ...
        <Text color="#3FB950" bold>{filtered.connected.length} </Text>
        <Text color="#484F58">│ OFFLINE: </Text>
        <Text color="#F85149" bold>{filtered.disconnected.length} </Text>
      ...
      {allList.length === 0 && (
        <Box paddingY={2} alignItems="center">
          <Text color="#FF5555" bold> [!] NO_MCP_NODES_DETECTED </Text>
      ...
      <Box flexDirection="column">
        {filtered.connected.length > 0 && (
          <Box marginTop={1} marginBottom={1}>
            <Text color="#3FB950" bold>▼ CONNECTED ─── {filtered.connected.length} nodes</Text>
          </Box>
        )}
        
        {visibleItems.filter(s => s.status === 'connected').map((s, i) => {
            ...
            <Box key={s.name} paddingLeft={2} backgroundColor={focused ? '#161b22' : undefined}>
              ...
              <Box flexDirection="row" justifyContent="space-between" flexGrow={1}>
                <Box>
                  ...
                  <Text color={focused ? 'white' : '#8C959F'} bold={focused}>{s.name.toUpperCase()}</Text>
        ...
        {filtered.disconnected.length > 0 && (
          <Box marginTop={1} marginBottom={1}>
            <Text color="#F85149" bold>▼ DISCONNECTED ─── {filtered.disconnected.length} nodes</Text>
          </Box>
        )}
      ...
      {confirmDelete && (
        <Box
          ...
          borderColor="#FF5555"
        >
          <Text color="#FF5555" bold> PURGE MCP NODE {confirmDelete.toUpperCase()}? [Y/N] </Text>
        </Box>
      )}

      <Box marginTop={1} justifyContent="space-between" paddingX={1}>
        <Text color="#484F58"> ESC:ABORT │ A:ADD │ ↑↓:NAVIGATE </Text>
        <Text color="#00E5FF" bold> E:EDIT │ ENTER:CONNECT/EDIT </Text>
      </Box>
    </Box>
  );
}
